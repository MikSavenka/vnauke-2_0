...
<div id="header" class="innerpage">
        	...
        	<div class="container940">
        		<p style="font-color:#fff !important; line-height:1.3em;" class="pagetitle">{FORUMS_POSTS_PAGETITLE}<p>
			...
			<form id="movetopic" action="{FORUMS_POSTS_MOVE_URL}" method="post" class="marginbottom10">
				<table class="flat">
					<tr>
						...
						<td class="width90">
							<a class="button" href="{FORUMS_POSTS_BUMP_URL}" title="{PHP.L.forums_explainbump}">{PHP.L.forums_bump}</a>
							<a class="button" href="{FORUMS_POSTS_LOCK_URL}" title="{PHP.L.forums_explainlock}">{PHP.L.Lock}</a> 
							<a class="button" href="{FORUMS_POSTS_STICKY_URL}" title="{PHP.L.forums_explainsticky}">{PHP.L.forums_makesticky}</a> 
							<a class="button" href="{FORUMS_POSTS_ANNOUNCE_URL}" title="{PHP.L.forums_explainannounce}">{PHP.L.forums_announcement}</a> 
							<a class="button" href="{FORUMS_POSTS_PRIVATE_URL}" title="{PHP.L.forums_explainprivate}">{PHP.L.forums_private} (#)</a> 
							<a class="button" href="{FORUMS_POSTS_CLEAR_URL}" title="{PHP.L.forums_explaindefault}">{PHP.L.Default}</a> 
							<a class="btn btn-warning" href="{FORUMS_POSTS_DELETE_URL}" title="{PHP.L.forums_explaindelete}" class="confirmLink">{PHP.L.Delete}</a>
						</td>
					</tr>
					<tr>
			...
			<table class="cells">
				...
				<tr>
					<td class="{FORUMS_POSTS_ROW_ODDEVEN}">
						...
						<h4><img src="themes/{PHP.theme}/img/online{FORUMS_POSTS_ROW_USERONLINE}.png" title="{PHP.L.Status}: {FORUMS_POSTS_ROW_USERONLINETITLE}" alt="" class="userstatus" /><strong>{FORUMS_POSTS_ROW_USERNAME}</strong></h4>
					</td>
					<td class="small centerall textright {FORUMS_POSTS_ROW_ODDEVEN}">
						<a name="{FORUMS_POSTS_ROW_ID}" id="{FORUMS_POSTS_ROW_POSTID}" href="{FORUMS_POSTS_ROW_IDURL}" rel="nofollow">Пост № {FORUMS_POSTS_ROW_ORDER}</a><span class="spaced"> / </span> Дата: {FORUMS_POSTS_ROW_CREATION}<!-- IF {FORUMS_POSTS_ROW_POSTERIP} --><span class="spaced">{PHP.cfg.separator}</span>{FORUMS_POSTS_ROW_POSTERIP}<!-- ENDIF --><!-- IF {FORUMS_POSTS_ROW_QUOTE} --><span class="spaced">{PHP.cfg.separator}</span>{FORUMS_POSTS_ROW_QUOTE}<!-- ENDIF --><!-- IF {FORUMS_POSTS_ROW_EDIT} --><span class="spaced">{PHP.cfg.separator}</span>{FORUMS_POSTS_ROW_EDIT}<!-- ENDIF --><!-- IF {FORUMS_POSTS_ROW_DELETE} --><span class="spaced">{PHP.cfg.separator}</span>{FORUMS_POSTS_ROW_DELETE}<!-- ENDIF --> {FORUMS_POSTS_ROW_BOTTOM}
					</td>
				</tr>
				<tr>
					<td class="{FORUMS_POSTS_ROW_ODDEVEN}">
						<!-- IF {FORUMS_POSTS_ROW_USERAVATAR} -->{FORUMS_POSTS_ROW_USERAVATAR}<!-- ELSE -->{PHP.R.forums_noavatar}<!-- ENDIF -->
						<p>Группа: {FORUMS_POSTS_ROW_USERMAINGRP}</p>
<p>Отправить ЛС: <a href="http://vim-studio.by/pm.php?m=send&to={FORUMS_POSTS_ROW_USERID}"><img src="http://vim-studio.by/images/icons/default/pm.png" /></a><p>
					</td>
					<td class="{FORUMS_POSTS_ROW_ODDEVEN}">
						<div>
...
<!-- BEGIN: FORUMS_POSTS_NEWPOST -->
		<form action="{FORUMS_POSTS_NEWPOST_SEND}" method="post" name="newpost">
			<table class="flat">
				...
				<tr>
					<td class="valid"><button class="btn btn-success" width=300px type="submit">{PHP.L.Reply}</button></td>
				</tr>
			</table>
		</form>
